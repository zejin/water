...
int run_sim(lua_State* L)
{
    ...
    int nthread = lget_int(L, "nthread", 1);
    int nbatch = lget_int(L, "nbatch", 1);
    const char* scale = lget_string(L, "scale", "weak");     
    
    central2d_t* sim = central2d_init(w,h, nx,ny,
                                      3, shallow2d_flux, shallow2d_speed, cfl, nbatch);
    lua_init_sim(L,sim);
    ...
    char* nthreads = (char*) malloc(3);
    sprintf(nthreads, "%d", nthread);
    ...
    FILE* fp;
    fp = fopen(fs, "w");
    fprintf(fp, "nthread,nbatch,ncell,time\n");

    double tcompute = 0;
    for (int i = 0; i < frames; ++i) {
#ifdef _OPENMP
        double t0 = omp_get_wtime();
        int nstep = central2d_run(sim, ftime, nthread, nbatch);
