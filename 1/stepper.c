                   ...
                   float tfinal, float dx, float dy, float cfl)
{
    ...
    float** f_block = (float**) malloc(NUM_THREAD*sizeof(float*));
    float** g_block = (float**) malloc(NUM_THREAD*sizeof(float*));
    float** u_block = (float**) malloc(NUM_THREAD*sizeof(float*));
    float** v_block = (float**) malloc(NUM_THREAD*sizeof(float*));
    float** scr_block = (float**) malloc(NUM_THREAD*sizeof(float*));
  
    int block_size = nx / NUM_THREAD;
    
    for(int i = 0; i < NUM_THREAD; ++i) {
        u_block[i] = (float*) malloc(nx_all*nfield*(2*NUM_BATCH+block_size)*sizeof(float));
        v_block[i] = (float*) malloc(nx_all*nfield*(2*NUM_BATCH+block_size)*sizeof(float));
        f_block[i] = (float*) malloc(nx_all*nfield*(2*NUM_BATCH+block_size)*sizeof(float));
        g_block[i] = (float*) malloc(nx_all*nfield*(2*NUM_BATCH+block_size)*sizeof(float));
    ...
    while (!done) {
        ...
        if (t + NUM_BATCH*dt >= tfinal) {
            dt = (tfinal-t)/NUM_BATCH;
            done = true;
        }
