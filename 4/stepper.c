                   ...
                   float tfinal, float dx, float dy, float cfl, int nthread, int nbatch)
{
    ...
    //sim->u  = (float*) malloc((4*N + 6*nx_all)* sizeof(float));
    float* restrict v  = u +   N;
    float* restrict f  = u + 2*N;
    float* restrict g  = u + 3*N;
    float* restrict scratch = u + 4*N;
...
/*    if (mic) {
        ...
        {
            nstep = central2d_yrun(u, v, scratch, f, g, nx, ny, ng, nfield, flux, speed, tfinal, dx, dy, cfl, nthread, nbatch);
        }
